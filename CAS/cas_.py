...
@njit
def _get_community_degree(node, label, labels_indptr, labels_indices, adjacency_indptr, adjacency_indices, adjacency_data):
...
@njit
def _ief(node, label, label_volume, labels_indptr, labels_indices, adjacency_indptr, adjacency_indices, adjacency_data):
    degree = _get_degree(node, adjacency_indptr, adjacency_indices, adjacency_data)
    if degree == 0:
        return 0
    in_community_degree = _get_community_degree(node, label, labels_indptr, labels_indices, adjacency_indptr, adjacency_indices, adjacency_data)
    return in_community_degree / degree


@njit
def _nief(node, label, label_volume, labels_indptr, labels_indices, adjacency_indptr, adjacency_indices, adjacency_data):
    degree = _get_degree(node, adjacency_indptr, adjacency_indices, adjacency_data)
    if degree == 0:
        return 0
    in_community_degree = _get_community_degree(node, label, labels_indptr, labels_indices, adjacency_indptr, adjacency_indices, adjacency_data)
...
def _p(node, label, label_volume, labels_indptr, labels_indices, adjacency_indptr, adjacency_indices, adjacency_data):
    #TODO
...
def _post_process_remove(
    ...
    for round_number in range(max_rounds):
        ...
        for label in range(len(labels_indptr)-1):
            ...
            label_volume = _get_volume(nodes, adjacency_indptr, adjacency_indices, adjacency_data) / graph_volume
            for data_offset, node in enumerate(nodes):
                cas_scores[labels_indptr[label]+data_offset] = cas(node, label, label_volume, labels_indptr, labels_indices, adjacency_indptr, adjacency_indices, adjacency_data)
        ...
        labels_indptr, labels_indices, labels_data = _eliminate_zeros(labels_indptr, labels_indices, labels_data)
...
def _get_nodes_to_move(
    ...
    move_action = np.empty_like(nodes_to_move, dtype="bool")  # True for add, False for remove
    ...
    label_volumes = np.empty(len(labels_csr_indptr)-1, dtype="float32")  # cache label volumes
    for label in range(len(labels_csr_indptr)-1):
        label_nodes = labels_csr_indices[labels_csr_indptr[label]:labels_csr_indptr[label+1]]
        label_volumes[label] = _get_volume(label_nodes, adjacency_indptr, adjacency_indices, adjacency_data) / graph_volume
    ...
    for node in range(len(adjacency_indptr)-1):
        current_labels = set(labels_csc_indices[labels_csc_indptr[node]:labels_csc_indptr[node+1]])
        plausible_labels = current_labels.copy()
        neighbors = adjacency_indices[adjacency_indptr[node]:adjacency_indptr[node+1]]
        for neighbor in neighbors:
            for neighbor_label in labels_csc_indices[labels_csc_indptr[neighbor]:labels_csc_indptr[neighbor+1]]:
                plausible_labels.add(neighbor_label)
        
        for label in plausible_labels:
            score = cas(node, label, label_volumes[label], labels_csr_indptr, labels_csr_indices, adjacency_indptr, adjacency_indices, adjacency_data)
            distance_to_threshold = abs(score - threshold)
            if  distance_to_threshold > min_distance_to_threshold: # move node into / out of label if score distance to theshold is sufficient
                if number_of_moves < max_per_round:  # empty spots
                    ...
                    number_of_moves += 1
                    if number_of_moves == max_per_round:  # array is full, compute min distance
                        min_distance_index = np.argmin(cas_threshold_distance)
                        min_distance_to_threshold = cas_threshold_distance[min_distance_index]
...
def _update_labels(
    ...
    new_indices = np.empty(len(labels_indices)+change_in_length, dtype=labels_indices.dtype)
    move_index_range = np.arange(len(move_action))

    next_index = 0
    for label in range(len(labels_indptr)-1):
        current_nodes = set(labels_indices[labels_indptr[label]:labels_indptr[label+1]])
...
def _post_process(
    ...
    #TODO randomly select for ties
    graph_volume = np.sum(adjacency_data)
    labels_csc_indptr, labels_csc_indices = _csr_to_csc(labels_csr_indptr, labels_csr_indices, len(adjacency_indptr)-1)
    
    for round_number in range(max_rounds):
        # Get nodes to move
        number_of_moves, nodes_to_move, labels_for_nodes, move_action = _get_nodes_to_move(
            labels_csr_indptr,
            labels_csr_indices,
            labels_csc_indptr,
            labels_csc_indices,
        ...
        if number_of_moves == 0:
            break
        labels_csr_indptr, labels_csr_indices = _update_labels(labels_csr_indptr, labels_csr_indices, nodes_to_move, labels_for_nodes, move_action)
        labels_csc_indptr, labels_csc_indices = _csr_to_csc(labels_csr_indptr, labels_csr_indices, len(adjacency_indptr)-1)
...
def _sparse_labels_to_numpy(labels_indptr, labels_indices, n_nodes):
    labels_csc_indptr, labels_csc_indices = _csr_to_csc(labels_indptr, labels_indices, n_nodes)
    ...
    for node in range(len(labels_csc_indptr)-1):
        if labels_csc_indptr[node] == labels_csc_indptr[node+1]:
            ...
            raise ValueError("Cannot convert a labels matrix with more than one label per node to a numpy array.")
    return new_labels
...
class CASTransformer:
    ...
    def fit(self, labels, adjacency):
        ...
        if isinstance(labels, np.ndarray):
            ...
            return_as_numpy = self.only_remove  # If passed a numpy array and only removing, return a numpy array
            ...
            raise ValueError(f"Expected labels to be a scipy sparse array of numpy array. Got {type(labels)}")

        if sp.issparse(adjacency):
            adjacency = adjacency.tocsr()
        else:
            raise ValueError(f"Adjacnecy must be a sparse matrix. Got {type(adjacency)}.")
        ...
        labels = sp.csr_matrix((labels_data, labels_indices, labels_indptr), shape=(len(labels_indptr)-1, adjacency.shape[0]), dtype="bool")
        labels.data[:] = True  # Sometime some entries are flipped to false, don't know why.
        
        if return_as_numpy:
            labels = _sparse_labels_to_numpy(labels.indptr, labels.indices, adjacency.shape[0])
